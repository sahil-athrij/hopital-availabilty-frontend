        ...
        return (
            <div className="card card-left neumorphic_input card-margin bg-grey">
                ...
                <div className="card-body card-body-left widget-49 bg-white w-100">
                    <h5 className="card-title card-title-left">{this.data.name}</h5>
                    <h6 className="card-title card-title-left">{this.data.display_address}</h6>
                    <div className="d-flex d-flex justify-content-between mb-3">
                        <h6 className="card-title card-title-left">
                            {
                                this.data.Phone === '0000000000' ? '' :
                        ...
                        <h6 className="card-title card-title-left" id="{{ id }}title">
                            <a target="_blank"
                               href={`https://www.google.com/maps/search/${this.data.name}/@${this.data.lat},${this.data.lng},19.88z`}>
                    ...
                    <div className="widget-49-meeting-points  row">
                        ...
                        <StarRating className="col-6" extra_className="financial"
                                    ...
                                    value={this.data.financial_rating}/>
                    </div>
                    <div className="widget-49-meeting-points  row">
                        <StarRating className="col-6" label="Convenience"
                                    ...
                                    disabled="disabled" value={this.data.care_rating}/>
                        <StarRating className="col-6" label="Covid Care"
                                    content="Quality of Covid Care (higher is better)" disabled="disabled"
                                    value={this.data.covid_rating}  />
                    </div>
                    <div className="widget-49-meeting-points mt-2 row">
                        <div className="col">Average Cost : <span>{this.data.avg_cost}</span> Rs</div>
                        <div className="col">Oxygen Availability : <span>{this.data.oxygen_availability}</span> %
                        </div>
                    </div>
                    <div className="widget-49-meeting-points my-2 row">
                        <div className="col">Ventilator Availability
                            : <span>{this.data.ventilator_availability}</span> %
                        </div>
                        <div className="col">ICU Availability : <span>{this.data.icu_availability}</span> %
                        </div>
                    </div>
                    <h6 className="card-title card-title-left" id="{{ id }}title">
                    ...
                    <div className="d-flex d-flex justify-content-between">
                        <a href={"/details/" + this.data.id + "?review=true"} className="btn bg-white text-warning">
                            <i className="fa fa-star"/>Share Feedback</a>

                        <a href={"/details/" + this.data.id} className="btn input-right input-left bg-dark btn-dark">More
                            Info</a>
                    </div>
                </div>
            </div>
